...
export function ModalAddConvoc({ children }) {
  ...
  const handleManualSubmit = async () => {
    if (!ciclo || !fechaInicio || !fechaFin) return;

    const selectedTemplates = Object.entries(templateSettings)
      .filter(([, value]) => value.selected)
      .map(([templateId, value]) => {
        const template = MOCK_TEMPLATES.find((item) => String(item.id) === templateId);
        return {
          template_id: Number(templateId),
          titulo: template?.titulo,
          fecha: value.date || null,
        };
      });
    ...
    const data = { 
        ...
        fecha_fin: fechaFin, 
        useTemplate, 
        plantillas: useTemplate ? selectedTemplates : [],
    };
