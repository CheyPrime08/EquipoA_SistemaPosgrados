"""
Servicio de Documentos - Maneja archivos y base de datos.
"""
...
class DocumentoService:
    def __init__(self):
        self.collection = db["reportes_documentos"]
        self._simulacion_db = []
    ...
    async def guardar_archivo(self, archivo: UploadFile, tipo: str, email: str = None) -> dict:
        ...
        documento = Documento(
            ...
            url_descarga=f"/static_documentos/{nombre_seguro}",
            tipo=tipo,
            usuario_email=email
        )
    ...
    def eliminar_documento(self, id_doc: str) -> bool:
        if self._is_db_available():
            # Primero obtener para saber la ruta del archivo y borrarlo?
            # Por ahora solo borrar registro
            resultado = self.collection.delete_one({"_id": ObjectId(id_doc)})
